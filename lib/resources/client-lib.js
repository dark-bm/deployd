...
ClientLib.prototype.generateResource = function(r, res) {
  ...
  if (r.clientGeneration && jsName) {
    ...
    res.write('  return dpd.on("' + r.path.replace(/^\//,'').replace(/\//g, '_') + '" + ":" + ev, fn);\n');
    res.write('}\n');
    res.write('dpd.' + jsName + '.once = function(ev, fn) {\n');
    res.write('  return dpd.once("' + r.path.replace(/^\//,'').replace(/\//g, '_') + '" + ":" + ev, fn);\n');
    res.write('}\n');
    res.write('dpd.' + jsName + '.off = function(ev, fn) {\n');
    res.write('  return dpd.off("' + r.path.replace(/^\//,'').replace(/\//g, '_') + '" + ":" + ev, fn);\n');
    res.write('}\n');
  }

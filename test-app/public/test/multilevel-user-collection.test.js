...
describe('Multilevel User Collection', function() {
	describe('dpd.securityusers', function() {
		describe('.post', function() {
			it('should create a user', function(done) {
				dpd.securityusers.post(credentials, function (user, err) {
					if(!user) {
			...
			it('should validate for duplicate username', function(done) {
				chain(function(next) {
					dpd.securityusers.post(credentials, next);
				}).chain(function(next) {
					dpd.securityusers.post(credentials, next);
      ...
      it('should properly receive emitToUsers messages', function(done) {
        dpd.socketReady(function() {
          dpd.securityusers.once('created', function(u) {
            expect(u).to.contain({ username: 'foo2' });
            done();
          });
        });

        dpd.securityusers.post({ username: 'foo', password: 'bar', admin: true })
        .then(function(res){
          expect(res).to.exist;
          return dpd.securityusers.login({ username: 'foo', password: 'bar'});
        })
        .then(function(res){
          dpd.securityusers.post({ username: 'foo2', password: 'bar' });
        });
      });

      it('should properly show username and password errors', function(done) {
        dpd.securityusers.post({}, function(res, err) {
      ...
      it('should update if id is passed in the body', function(done) {
        chain(function(next) {
          dpd.securityusers.post({username: 'foo', password: 'bar'}, next);
        }).chain(function(next, res, err) {
          dpd.securityusers.post({id: res.id, username: 'test'}, next);
        }).chain(function(next, res, err) {
          done(err);
        });
      });
      ...
      it('should update if id is passed in the url', function(done) {
        chain(function(next) {
          ...
          dpd.securityusers.post(res.id, {username: 'test'}, next);
        }).chain(function(next, res, err) {
          done(err);
        });
      });
		...
		describe('.login(credentials, fn)', function() {
			it('should login a user', function(done) {
				dpd.securityusers.post(credentials, function (user, err) {
					expect(user.id.length).to.equal(16);
					dpd.securityusers.login(credentials, function (session, err) {
      ...
      it('should not crash the server when called without a body', function(done) {
        dpd.securityusers.login(null, function(session, err) {
          expect(err).to.exist;
          done();
        });
      });
      ...
      it('should not crash the server when called without a password', function(done) {
        dpd.securityusers.post({username: 'foo@bar.com', password: '123456'})
        .then(function(res) {
          expect(res).to.exist;
          expect(res.username).to.equal('foo@bar.com');
          dpd.securityusers.login({username: 'foo@bar.com'}, function(session, err) {
            expect(err).to.exist;
            done();
          });
        });
      ...
      it('should call login event and provide access to user in event', function(done) {
        dpd.securityusers.post(credentials, function (user, err) {
          ...
          dpd.socketReady(function() {
            dpd.securityusers.once('test_event', function(u) {
      ...
      it('should allow canceling login from login event', function (done) {
        dpd.securityusers.post({username: 'foo@bar.com', password: '123456'}, function (user, err) {
          expect(user.id.length).to.equal(16);
          dpd.securityusers.login({username: 'foo@bar.com', password: '123456', authtoken: '$BAD_AUTH'}, function (session, err) {
            expect(err).to.exist;
      ...
      it('should allow updating the user from the login event', function (done) {
        dpd.securityusers.post({ username: 'foo2@bar.com', password: '123456' }, function (user, err) {
          expect(user.id.length).to.equal(16);
          expect(user.lastLoginTime).to.not.exist;
          dpd.securityusers.login({ username: 'foo2@bar.com', password: '123456' }, function (session, err) {
            expect(session.id.length).to.equal(128);
            expect(session.uid.length).to.equal(16);
            expect(err).to.not.exist;
            dpd.securityusers.get(session.uid, function (user, err) {
              expect(err).to.not.exist;
      ...
      it('should allow updating the user from the login event when login fails', function (done) {
        dpd.securityusers.post({ username: 'foo3@bar.com', password: '123456' }, function (user, err) {
          expect(user.id.length).to.equal(16);

          // try 4 bad logins; the logic in the login event will ban the user after 3 failed attempts
          chain(function (next) {
            dpd.securityusers.login({ username: 'foo3@bar.com', password: 'bad' }, next);
            ...
            expect(session).to.not.exist;
            // remove ban
            dpd.securityusers.put(user.id, { banned: false }, next);
          }).chain(function (next, user, err) {
            expect(err).to.not.exist;
            // try a proper login this time
            dpd.securityusers.login({ username: 'foo3@bar.com', password: '123456' }, next);
          }).chain(function (next, session, err) {
            expect(session.id.length).to.equal(128);
            expect(session.uid.length).to.equal(16);
            expect(err).to.not.exist;
      ...
      it('should not call other events after update of user from login event', function (done) {
        dpd.securityusers.post({ username: '$SKIP_EVENTS_TEST', password: '123456' })
        .then(function (user) {
          expect(user.id.length).to.equal(16);
          return dpd.securityusers.login( { username: '$SKIP_EVENTS_TEST', password: '123456' });
        })
      ...
      it('should call login event even when user does not exist', function (done) {
        dpd.securityusers.login({ username: 'foo123456@bar.com', password: '123456' }, function (session, err) {
          expect(err).to.exist;
      ...
      it('should not crash server when login is called repeatedly', function (done) {
        ...
        dpd.securityusers.post({ username: 'foo@bar.com', password: '123456' }, function (user, err) {
          ...
          function doLogin() {
            dpd.securityusers.login({ username: 'foo@bar.com', password: '123456' }, loginDone);
          }
      ...
      it('should allow login with Authorization: Bearer HTTP header ', function (done) {
        ...
        dpd.securityusers.logout(function() {
          // setting this header will disable setting a sid cookie by deployd
          _dpd.ajax.headers = { Authorization: "Bearer" };
          dpd.securityusers.post({ username: 'authheader', password: '123456' }, function (user, err) {
            expect(user.id.length).to.equal(16);
            expect(user.lastLoginTime).to.not.exist;
            dpd.securityusers.login({ username: 'authheader', password: '123456' }).then(function (session) {
              _dpd.ajax.headers = { Authorization: "Bearer " + session.id };
              dpd.securityusers.me({showUsername: true}, function (me, err) {
		...
		describe('.me(fn)', function() {
			it('should return the current user', function(done) {
				dpd.securityusers.post(credentials, function (user, err) {
					expect(user.id.length).to.equal(16);
					dpd.securityusers.login(credentials, function (session, err) {
						dpd.securityusers.me(function (me, err) {
							expect(me).to.exist;
      ...
      it('should invalidate session if username or password is changed', function (done) {
        dpd.securityusers.post({ username: 'foo123@bar.com', password: '123456' }).then(function (user) {
          expect(user.id.length).to.equal(16);
          return dpd.securityusers.login({ username: 'foo123@bar.com', password: '123456' });
        })
        .then(function (session) {
          expect(session).to.exist;
          expect(session.id.length).to.equal(128);
          return dpd.securityusers.me();
        })
        // test changing username:
        .then(function (user) {
          return dpd.securityusers.put(user.id, { username: 'foo1234@bar.com' });
        })
        .then(function (user) {
        ...
        .then(function (user) {
          ...
          return dpd.securityusers.login({ username: 'foo1234@bar.com', password: '123456' });
        })
        .then(function (session) {
          expect(session).to.exist;
          expect(session.id.length).to.equal(128);
        ...
        .then(function (user) {
          return dpd.securityusers.put(user.id, { password: '1234567' });
        })
        .then(function (user) {
          expect(user).to.exist;
          return dpd.securityusers.me();
        ...
        .then(function (user) {
          expect(user).to.equal('');
          return dpd.securityusers.login({ username: 'foo1234@bar.com', password: '1234567' });
        })
        .then(function (session) {
          expect(session).to.exist;
          expect(session.id.length).to.equal(128);
		...
		describe('.del({id: \'...\'}, fn)', function() {
			it('should remove a user', function(done) {
				dpd.securityusers.post(credentials, function (user, err) {
					expect(user.id.length).to.equal(16);
					dpd.securityusers.del({id: user.id}, function (res, err) {
						expect(err).to.not.exist;
						expect(res.count).to.equal(1);
						dpd.securityusers.get({id: user.id}, function (user) {
		...
		describe('dpd.securityusers.on("changed", fn)', function() {
      it('should respond to the changed event (in AfterCommit) on post', function(done) {
        dpd.socketReady(function() {
          dpd.securityusers.once('changed', function() {
            done();
          });
      ...
      it('should respond to the changed event (in AfterCommit) on put', function(done) {
        dpd.securityusers.post({username: 'foo2@bar.com', password: '123456'}, function(item) {
          dpd.socketReady(function() {
            dpd.securityusers.once('changed', function() {
              done();
            });

            dpd.securityusers.put(item.id, {username: 'foo3@bar.com'});
          });
        });
      });
      ...
      it('should respond to the changed event (in AfterCommit) on del', function(done) {
        dpd.securityusers.post({username: 'foo2@bar.com', password: '123456'}, function(item) {
          dpd.socketReady(function() {
            ...
            dpd.securityusers.del(item.id);
          });
        });
      });
    });

    describe('dpd.securityusers.put({}, fn)', function() {
      it('should allow omitting username and password', function(done) {
        chain(function(next) {
          dpd.securityusers.post({username: 'foo', password: 'bar'}, next);
        }).chain(function(next, res, err) {
          dpd.securityusers.put(res.id, {reputation: 10}, next);
        }).chain(function(next, res, err) {
      ...
      it('should not allow nulled password', function(done) {
        ...
        chain(function(next) {
          ...
          uid = res.id;
          dpd.securityusers.login(credentials, next);
        }).chain(function(next, res, err) {
          dpd.securityusers.put(uid, {password: null}, next);
        }).chain(function(next, res, err) {
      ...
      it('should not allow empty password string', function(done) {
        ...
        chain(function(next) {
          ...
          dpd.securityusers.put(uid, {password: ''}, next);
        }).chain(function(next, res, err) {
          expect(err).to.exist.with.property('errors');
          expect(err.errors).to.have.property('password');
          done(res);
      ...
      it('should not allow unauthenticated changes to username or password', function(done) {
        chain(function(next) {
          ...
          dpd.securityusers.put(res.id, {username: 'changed', password: 'changed'}, next);
        }).chain(function(next, res, err) {
          expect(res.username).to.equal('foo');
          dpd.securityusers.login({username: 'changed', password: 'changed'}, next);
        }).chain(function(next, res, err) {
          expect(err).to.exist;
          done();
        });
      ...
      it('should allow authenticated changes to username or password', function(done) {
        ...
        chain(function(next) {
          ...
          id = res.id;
          dpd.securityusers.login({username: 'foo', password: 'bar'}, next);
        }).chain(function(next) {
          dpd.securityusers.put(id, {username: 'changed', password: 'changed'}, next);
        }).chain(function(next, res, err) {
          if(err) return done(err);
      ...
      it('should allow changes to username and password via events', function(done) {
        chain(function(next) {
          ...
          if(err) return done(err);
          dpd.securityusers.put(res.id, {displayName: "$CHANGEPASSWORD"}, next);
        }).chain(function(next, res, err) {
          if(err) return done(err);
          dpd.securityusers.login({username: 'foo', password: 'changed'}, next);
        }).chain(function(next, res, err) {
          if(err) return done(err);
          done();
        });
      ...
      it('should return true for isMe()', function(done) {
        ...
        chain(function(next) {
          ...
          dpd.securityusers.put(id, {displayName: "Foo Bar!"}, next);
        }).chain(function(next, res, err) {
          if(err) return done(err);
    ...
    afterEach(function (done) {
      ...
      dpd.securityusers.logout(function () {
        // delete all users
        dpd.securityusers.del({id: { $ne: null } }, function (users) {
          done();
        });
      });
    });
